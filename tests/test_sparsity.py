...
The colouring is half of what a sparsity pattern buys -- ``n_colours + 1`` Enzyme
sweeps rather than ``n_vars + 1``. The other half, the sparse direct solver the
same pattern compiles, lives in ``test_sparse_direct.py``.
"""
...
@requires_cuda
def test_a_pattern_solves_what_no_pattern_solves():
    """A pattern changes the sweeps and the factorisation, not the answer."""
    compressed = colour_sparsity(normalize_sparsity(CHAIN, N))
    ...
    sparse = np.asarray(solve(chain_ode, y0, t, p, sparsity=CHAIN, **kw))
...
@requires_cuda
def test_gradients_survive_compression():
    ...
    sparse = jax.grad(loss)(p0, sparsity=CHAIN)
